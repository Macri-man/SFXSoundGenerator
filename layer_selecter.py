from PyQt6.QtWidgets import QWidget, QHBoxLayout, QPushButton, QComboBox, QLabel, QInputDialog
import copy
from layer import Layer

class LayerSelector(QWidget):
    """Manage layers: add, delete, rename, copy/paste."""
    def __init__(self, layers, layer_changed_callback):
        ...
        # Buttons
        self.rename_btn = QPushButton("Rename"); self.rename_btn.clicked.connect(self.rename_layer)
        self.copy_btn = QPushButton("Copy"); self.copy_btn.clicked.connect(self.copy_layer)
        self.paste_btn = QPushButton("Paste"); self.paste_btn.clicked.connect(self.paste_layer)
        self.add_btn = QPushButton("Add"); self.add_btn.clicked.connect(self.add_layer)
        self.delete_btn = QPushButton("Delete"); self.delete_btn.clicked.connect(self.delete_layer)
    ...
    def update_selector(self):
        ...
        for i, layer in enumerate(self.layers):
            ...
            self.selector.addItem(f"{i+1}: {name}")
    ...
    def change_layer(self, index):
        if 0 <= index < len(self.layers):
            ...
            self.layer_changed_callback(index)
    ...
    def rename_layer(self):
        ...
        text, ok = QInputDialog.getText(self, "Rename Layer", "New name:", text=getattr(layer,"name",""))
